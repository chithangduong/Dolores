...
def read_graph():
	...
	weighted = 0
	relationed = 1
	graph_input = "/u01/why/ucsb/node2vec/graph/train2id.edgelist"
	directed = 0
	if weighted:
...
def calcu_METRIC(logits):
    ...
    '''
    ent_num = 14541
    with open("/u01/why/ucsb/benchmarks/FB15K237/test2id.txt",'r') as f:
        lines = f.readlines()
    ...
    def calculate_metrics(L_or_R, rank_list, rank_recipr_list, rank_list_filter, rank_recipr_list_filter):
        ...
        #with open("/home/why2011btv/2018-08-02_21-00-02predicted_logit.txt", 'rb') as file_logit:
            ...
            for i in range(batch_num):
                ...
                if (i+1) % 100 == 0:
                    ...
                    print("MRR for tail substitution with no type constraint: %f" % (sum(rank_recipr_list_filter)/len(rank_recipr_list_filter)))
        ...
        path_OUT = time_now + './L_or_R' + str(L_or_R) + "_list_of_rank.txt"
        with open(path_OUT,'w') as f_out:

            for item in rank_list_filter:
                f_out.write("%s\n" % item)
